...
export default function CarritoPage() {
  ...
  const lineFor = (productId: string) =>
    quote?.lines.find((l) => l.productId === productId) ?? null;
      ...
      <div className="mt-8 grid grid-cols-1 gap-12 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-6">
          {items.map(({ product, quantity }) => {
            ...
            const unit = line?.unitFinal ?? product.price;
            const hasDiscount = !!line?.hasDiscount;
            return (
              <div
                key={product.id}
                    ...
                    <p className="text-sm text-gray-500">
                      {hasDiscount && (
                        <span className="mr-1.5 text-gray-400 line-through">
                          {formatPrice(line!.unitList)}
                        </span>
                      )}
                      {formatPrice(unit)} c/u
                    </p>
                    {line?.promoName && (
                      <p className="text-xs font-medium text-save">🏷️ {line.promoName}</p>
                    )}
                  </div>
        ...
        <div className="rounded-lg border p-6 h-fit">
          ...
          <div className="mt-4 space-y-2 text-sm">
            <div className="flex justify-between">
              <span>Productos ({quote?.count ?? itemCount})</span>
              <span>{formatPrice(quote?.listSubtotal ?? total)}</span>
            </div>
            ...
            <div className="flex justify-between">
              <span>Envío</span>
              <span className="text-gray-500">A coordinar</span>
            </div>
          </div>
          ...
          {quote && quote.saving > 0 && (
            <p className="mt-2 text-right text-sm font-semibold text-save">
              Ahorrás {formatPrice(quote.saving)}
            </p>
          )}
          <Link
            href="/checkout"
            className="mt-6 block w-full rounded-md bg-brand py-3 text-center text-sm font-medium text-white hover:bg-brand-dark"
          >
            Ir al checkout
          </Link>
        </div>
